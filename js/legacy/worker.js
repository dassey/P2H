...
function processPPTX(data) {
    ...
    filesInfo["slides"] = getOrderedSlidePaths(zip, filesInfo["slides"]);
...
// [Content_Types].xml lists slide parts in file order, which is NOT the
// presentation order once slides have been moved or deleted in PowerPoint.
// Resolve the true order from p:sldIdLst via the presentation rels.
function getOrderedSlidePaths(zip, fallback) {
    try {
        var pres = readXmlFile(zip, "ppt/presentation.xml");
        var rels = readXmlFile(zip, "ppt/_rels/presentation.xml.rels");
        var relArr = rels["Relationships"]["Relationship"];
        if (relArr.constructor !== Array) { relArr = [relArr]; }
        var relMap = {};
        for (var i = 0; i < relArr.length; i++) {
            relMap[relArr[i]["attrs"]["Id"]] = relArr[i]["attrs"]["Target"];
        }
        var lst = getTextByPathList(pres, ["p:presentation", "p:sldIdLst", "p:sldId"]);
        if (lst === undefined) { return fallback; }
        if (lst.constructor !== Array) { lst = [lst]; }
        var ordered = [];
        for (i = 0; i < lst.length; i++) {
            var target = relMap[lst[i]["attrs"]["r:id"]];
            if (target === undefined) { continue; }
            ordered.push(target.charAt(0) === "/" ? target.substr(1) : "ppt/" + target);
        }
